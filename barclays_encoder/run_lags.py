...
# sel = [5,7]
result_file = 'barclays_lags_result.txt'
...
def pred_train(in_dim=1, hidden_dim=64, n_reg=1, num_pred=3, num_lags=9):
    ...
    for epoch in range(20):
        ...
        if val_mse < min_val_mse:
            min_val_mse = val_mse
            torch.save(model, 'pred_train_barclays_lags.pkl')

    return 'pred_train_barclays_lags.pkl'
...
def run_model(pred_train_path, hidden_dim=64, n_reg=1):
    ...
    for epoch in range(60):
        ...
        if val_mse < min_val_mse:
            min_val_mse = val_mse
            torch.save(model, 'barclays_lags.pkl')

    model = torch.load('barclays_lags.pkl')
    model.eval()
