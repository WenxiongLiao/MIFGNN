...
# sel = [5,7]
result_file = 'barclays_lags_weather_event_text_result.txt'
...
#         self.text_att = SelfAttention(int(hidden_dim / 2))
    ...
    def forward(self, lags, weather, event, text):
        ...
        text = torch.reshape(text, (-1, 80))  # max_length
...
def run_model(hidden_dim=100, lags_decay=0.12, weather_decay=0.28, event_decay= 0, b_decay=0, merge_decay=0.14, other_decay=0.13):
    # Create model
    ...
    for epoch in range(300):
        ...
        if val_mse < min_val_mse:
            min_val_mse = val_mse
            torch.save(model, 'barclays_lags_weather_event_text.pkl')

    model = torch.load('barclays_lags_weather_event_text.pkl')
    model.eval()
