...
sel = [5, 7]
result_file = 'termial5_lags_weather_event_result.txt'
...
trends = df_sum["trend"].shift(-1).as_matrix()
stds = df_sum["std"].shift(-1).as_matrix()
...
class MGN(nn.Module):
    def __init__(self, hidden_dim, activation):
        ...
        self.merge_linner = nn.Linear(hidden_dim * 3, hidden_dim)
    ...
    def message_func(self, edges):
        return {'l': edges.src['l'], 'w': edges.src['w'], 'e': edges.src['e']}
...
class Regression(nn.Module):
    def __init__(self, in_dim, hidden_dim, n_reg):
        ...
        self.lags_hidden128 = nn.Linear(in_dim, hidden_dim)
        self.lags_norm128 = nn.BatchNorm1d(hidden_dim)
        self.lags_hidden50 = nn.Linear(hidden_dim, hidden_dim)
        self.lags_norm50 = nn.BatchNorm1d(hidden_dim)

        self.weather_norm2 = nn.BatchNorm1d(2)
        self.weather_hidden128 = nn.Linear(2, hidden_dim)
        self.weather_norm128 = nn.BatchNorm1d(hidden_dim)
        self.weather_hidden50 = nn.Linear(hidden_dim, hidden_dim)
        self.weather_norm50 = nn.BatchNorm1d(hidden_dim)

        self.event_norm4 = nn.BatchNorm1d(4)
        self.event_hidden128 = nn.Linear(4, 128)
        self.event_norm128 = nn.BatchNorm1d(128)
        self.event_hidden50 = nn.Linear(128, hidden_dim)
        self.event_norm50 = nn.BatchNorm1d(hidden_dim)
...
def model_test(data_loader, model):
    ...
    for step, (lags, weather, event, test_y_batch) in enumerate(data_loader):
        lags = lags.cuda(DEVICE)
        weather = weather.cuda(DEVICE)
        event = event.cuda(DEVICE)
        test_y_batch = test_y_batch.numpy()

        pred_y_batch = model(lags, weather, event)
...
def run_model(hidden_dim=128, lags_decay=0.1, weather_decay=0.18, event_decay=0.02, b_decay=0, merge_decay=0.1,
              other_decay=0.12):
    # Create model
    ...
    for epoch in range(200):
        ...
        if val_mse < min_val_mse:
            min_val_mse = val_mse
            torch.save(model, 'terminal_dgl_lags_weather_event.pkl')

    model = torch.load('terminal_dgl_lags_weather_event.pkl')
    model.eval()
