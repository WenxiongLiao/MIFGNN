...
def run_model(hidden_dim=128,lags_decay=0.06, weather_decay=0.16, merge_decay=0, other_decay=0):
    # Create model
    ...
    for epoch in range(200):
        ...
        if val_mse < min_val_mse:
            min_val_mse = val_mse
            torch.save(model, 'terminal_lags_weather.pkl')

    model = torch.load('terminal_lags_weather.pkl')
    model.eval()
