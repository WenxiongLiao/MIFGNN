...
def build_model(num_inputs, num_lags, num_preds):
    ...
    preds = Dense(units=num_preds, kernel_regularizer=keras.regularizers.l2(0.1))(x)
...
def build_model_weather(num_inputs, num_lags, num_feat, num_preds):
    ...
    preds = Dense(units=num_preds, kernel_regularizer=keras.regularizers.l2(0.1))(feat)
...
embeddings_index = {}
f = open(GLOVE_DIR + 'glove.6B.%dd.txt' % (EMBEDDING_DIM,),encoding='utf-8')
for line in f:
    values = line.split()
    word = values[0]
...
def build_model_text(num_inputs, num_lags, num_feat, num_preds):
    ...
    #temp1 = Permute([1,2])(RepeatVector(360)(BatchNormalization()(text_embedding)))
    temp1 = Reshape((1,360))(text_embedding)
    temp1 = Permute([2,1])(temp1)
    print(("temp1:", temp1))
    
    print(("x_lags:", x_lags))
    temp2 = Permute([1,2])(RepeatVector(360)(BatchNormalization()(x_lags)))
    ...
    temp = Reshape((360,))(temp)
    temp = BatchNormalization()(temp)
    print(temp)
    attention_probs = Activation("softmax")(temp)
    #print fail

    #attention_probs = Dense(360, activation='softmax', name='attention_vec')(text_embedding)
    # attention_mul = merge([text_embedding, attention_probs], output_shape=360, name='attention_mul', mode='mul')
    attention_mul = keras.layers.Multiply(name='attention_mul')([text_embedding, attention_probs])
    attention_mul = BatchNormalization()(attention_mul)
    #attention_mul = Dropout(0.5)(attention_mul)
    
    feat = Concatenate(axis=1)([feat, attention_mul])
