...
sel = [5, 7]
result_file = 'termial5_lags_weather_result.txt'
...
class Regression(nn.Module):
    def __init__(self, hidden_dim, n_reg, pred_train_path):
        ...
        self.weather_hidden200 = nn.Linear(2, hidden_dim)
        ...
        self.regression = nn.Linear(hidden_dim * 2, n_reg)
    ...
    def forward(self, lags, weather):
        ...
        feature = torch.cat((lags_feature, weather_feature), 1)
...
def model_test(data_loader, model, is_pre_train=False):
    ...
    for step, (test_x_batch, test_weather_batch, test_y_batch) in enumerate(data_loader):
        ...
        if is_pre_train == False:
            pred_y_batch = model(test_x_batch, test_weather_batch)
        else:
...
#                             other_decay=0)
    ...
    for epoch in range(20):
        model.train()
        epoch_loss = 0
        for iter, (x,_, y) in enumerate(train_data_loader):
            x = x.cuda(DEVICE)
            y = y.cuda(DEVICE)
            prediction = model(x)
...
#         print(epoch,val_mse)
        if val_mse < min_val_mse:
            min_val_mse = val_mse
            torch.save(model, 'pred_train_terminal5_lags_weather.pkl')

    return 'pred_train_terminal5_lags_weather.pkl'
...
#         print(epoch, val_mse)
        if val_mse < min_val_mse:
            min_val_mse = val_mse
            torch.save(model, 'terminal5_lags_weather.pkl')

    model = torch.load('terminal5_lags_weather.pkl')
    model.eval()
