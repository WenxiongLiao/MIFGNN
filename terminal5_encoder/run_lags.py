...
class Regression(nn.Module):
    ...
    def forward(self, inputs):
        with torch.no_grad():
            feature = self.pred_train_model(inputs, extract_feature=True)
...
def pred_train(in_dim=1, hidden_dim=64, n_reg=1,num_pred=3,num_lags=10):
    # Create model
    model = Encoder_decoder(in_dim=1, hidden_dim=64, n_reg=1,num_pred=3,num_lags=10)
    model.to(DEVICE)
...
#         print(epoch,val_mse)
        if val_mse < min_val_mse:
            min_val_mse = val_mse
            torch.save(model, 'pred_train_terminal5_lags.pkl')

    return 'pred_train_terminal5_lags.pkl'
...
#         print(epoch, val_mse)
        if val_mse < min_val_mse:
            min_val_mse = val_mse
            torch.save(model, 'terminal5_lags.pkl')

    model = torch.load('terminal5_lags.pkl')
    model.eval()
