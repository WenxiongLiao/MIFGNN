...
GLOVE_DIR = "E:/语料/word2vec/glove.6B/"
...
for gp_lenscale in [0.1,1,10,100]:
    for epsilon in []:
    #for gp_noise in [0.01,0.1,1,10,100]:
        ...
        neigh = GaussianProcessRegressor(kernel=kernel, alpha=0.5)
        #neigh.fit(lags_train, y_train)
        #preds_lr = neigh.predict(lags_test)
        neigh.fit(np.concatenate([lags_train, event_feats_train, lags_event_feats_train, weather_feats_train[:,sel]], axis=1), y_train)
        preds_lr = neigh.predict(np.concatenate([lags_test, event_feats_test, lags_event_feats_test, weather_feats_test[:,sel]], axis=1))
...
kernel = gp_scale * RBF(length_scale=gp_lenscale, length_scale_bounds=(1e-2, 1e3)) + WhiteKernel(noise_level=gp_noise, noise_level_bounds=(1e-10, 1e+1))
gp = GaussianProcessRegressor(kernel=kernel, alpha=gp_alpha)
gp.fit(np.concatenate([lags_train, event_feats_train, lags_event_feats_train, weather_feats_train[:,sel]], axis=1), y_train)
preds_lr = gp.predict(np.concatenate([lags_test, event_feats_test, lags_event_feats_test, weather_feats_test[:,sel]], axis=1))
preds_lr = preds_lr * std_test + trend_test
y_true = y_test * std_test + trend_test
corr, mae, rae, rmse, rrse, mape, r2 = compute_error(y_true, preds_lr)
print("MAE:  %.3f\tRMSE: %.3f\tR2:   %.3f" % (mae, rmse, r2))
